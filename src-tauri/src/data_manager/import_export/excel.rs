...
pub struct ExcelDataImporter {
    ...
    history_service: Option<Arc<CalculationHistoryService>>,
}
...
impl ExcelDataImporter {
    ...
    pub fn with_history_service(
        ...
        Self {
            calculation_service,
            history_service: Some(history_service),
        }
    }

    pub async fn import_to_history(&self, path: &str) -> Result<String> {
        if self.history_service.is_none() {
            return Err(ImportError::InvalidFormat("No history service available".into()).into());
        }

        let (num_plates, imported_data, initial_mass, initial_concentration) =
            self.import(path).await?;

        if imported_data.is_empty() {
            return Err(ImportError::InvalidFormat("No valid data in Excel file".into()).into());
        }
        ...
        let history_id = self
            .create_history_from_entries(
                &imported_data,
                num_plates,
                initial_mass.unwrap_or(0.0),
                initial_concentration.unwrap_or(0.0),
            )
            .await?;
    ...
    async fn create_history_from_entries(
        &self,
        ...
        let entries = calculation_service.calculate_distilled_mass(
            initial_concentration,
            initial_mass,
            entries,
        );
        ...
        for entry in entries.iter() {
            history_service
                .record_step(entry.clone())
                .await
                .map_err(|e| ImportError::InvalidFormat(format!("Failed to record step: {}", e)))?;
        }

        let history_path = history_service
            .finish_current_history()
            .await
            .map_err(|e| ImportError::InvalidFormat(format!("Failed to finish history: {}", e)))?;

        let history_id = history_path
            .and_then(|path| {
                path.file_name()
    ...
    pub async fn import(
        ...
        let (number_plates, column_structure) = self.parse_headers(&range, header_row)?;

        let entries = self.process_rows(&range, &column_structure, header_row)?;

        Ok((number_plates, entries, initial_mass, initial_composition))
    }
    ...
    fn process_rows(
        ...
        for (index, row_index) in (start_row..row_count).enumerate() {
            ...
            let percentage_complete = if total_rows > 0 {
                (index as f64 + 1.0) / total_rows as f64 * 100.0
            } else {
                0.0
            };
            ...
            let compositions: Vec<CompositionResult>;

            if structure.has_compositions {
